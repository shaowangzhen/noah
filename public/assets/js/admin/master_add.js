...
$(function () {
    ...
    $('.btn_sub').on('click', function (e) {
        e.preventDefault();
        var ser = $('#searched').val();
        if(parseInt(ser) !== 1)
        {
            $('#tips_mastername').html('请输入用户名搜索!');
            $('#mastername').focus();
            return false;
        }
...
function formCheck() {
    ...
    if(nameChecked && passwordChecked && emailChecked) {
        $('#checked').val('1');
    } else {
        $('#checked').val('0');
    }
